...
const COMMANDS: Record<string, CommandHandler> = {
    ...
    'powerup': (args, sm) => {
        if (args.length < 1) return "Usage: /powerup <type> (instakill, max_ammo, double_points, nuke, carpenter)";
        ...
        if (!powerUpType) return `Unknown powerup type: ${args[0]}. Use: instakill, max_ammo, double_points, nuke, carpenter`;
