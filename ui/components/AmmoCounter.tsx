...
const BulletIndicator: React.FC<{ filled: boolean; index: number }> = ({ filled, index }) => (
    <div 
        className={`w-[3px] h-3 rounded-sm transition-all duration-150 ${
            filled 
...
export const AmmoCounter: React.FC<AmmoCounterProps> = ({ weaponName, ammo, reserveAmmo, maxClip }) => {
                    ...
                    <div className="flex gap-1">
                        {[...Array(3)].map((_, i) => (
                            <div key={i} className={`w-1 h-1 rounded-full transition-all duration-300 ${
                                i === 0 ? (isEmpty ? 'bg-red-500' : isLowAmmo ? 'bg-amber-500' : 'bg-stone-500') :
                                'bg-stone-700'
                            }`} />
                        ))}
                    </div>
